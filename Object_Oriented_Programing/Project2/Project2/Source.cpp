...
int main()
{
	...
	*it = "amazing";
	...
	evenNumbers = count_if(integers.begin(), integers.end(), [](int x) -> bool {return x % 2 == 0; }); // we can specify the return type
	...
	for_each(integers.begin(), integers.end(), [](int x) { cout << x << " "; }); // if the lambda returns a result, the result is discarded

	vector<int> doublesOfIntegers(5);
	transform(integers.begin(), integers.end(), doublesOfIntegers.begin(), [](int x) { return x * 2; }); // the return value is assigned to successive elements in the output range
